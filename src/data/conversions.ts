import { ConversionItem, LayerInfo } from "../types";
import {
  parseFlexibleDate,
  formatToTimezoneISO,
...
export const LAYERS: Record<string, LayerInfo> = {
  ...
  host: {
    ...
    description:
      "Sistema Legacy en Portugal. Almacena fecha/hora en horario Portugal (Europe/Lisbon). No puede guardar código ni offset de zona horaria.",
...
export const CONVERSION_CATALOG: ConversionItem[] = [
  // --- ZONA HORARIA ---
  {
    ...
    category: "timezone",
    description:
      "Convierte una fecha/hora de España a Portugal restando 1 hora y preservando la estructura del formato de entrada.",
    apply: (input: string) => {
  ...
  },
  {
    ...
    category: "timezone",
    description:
      "Convierte una fecha/hora de Portugal a España sumando 1 hora y preservando la estructura del formato de entrada.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
      if (parsed.hasTimezone && parsed.originalFormat === "iso") {
  ...
  },
  {
    ...
    category: "timezone",
    description:
      "Convierte la fecha/hora a la zona horaria de España (Europe/Madrid) con su offset real (+01:00 / +02:00).",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
      return formatToTimezoneISO(parsed.date, "Europe/Madrid");
  ...
  },
  {
    ...
    category: "timezone",
    description:
      "Convierte la fecha/hora a la zona horaria de Portugal (Europe/Lisbon) con su offset real (+00:00 / +01:00).",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
      return formatToTimezoneISO(parsed.date, "Europe/Lisbon");
  ...
  },
  {
    ...
    category: "timezone",
    description:
      "Interpreta la fecha en horario local de España y calcula su equivalente exacto en UTC ISO.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
  ...
  },
  {
    ...
    category: "timezone",
    description:
      "Interpreta la fecha en horario local de Portugal y calcula su equivalente exacto en UTC ISO.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
      if (!parsed.hasTimezone && parsed.hasTime) {
  ...
  // --- PRESENCIA DE HORA ---
  {
    ...
    description:
      "Elimina el componente de hora, conservando únicamente la fecha en formato YYYY-MM-DD.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) {
  ...
  },
  {
    ...
    category: "time_presence",
    description:
      "Si la entrada es una fecha simple (YYYY-MM-DD), añade medianoche con T (T00:00:00).",
    apply: (input: string) => {
      const trimmed = input.trim();
  ...
  },
  {
    ...
    category: "time_presence",
    description:
      "Si la entrada es una fecha simple (YYYY-MM-DD), añade medianoche separada por espacio (YYYY-MM-DD 00:00:00).",
    apply: (input: string) => {
      const trimmed = input.trim();
      if (/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
  ...
  },
  {
    ...
    category: "time_presence",
    description:
      "Sustituye o añade la hora actual del dispositivo del usuario a la fecha de entrada.",
    apply: (input: string) => {
      const now = new Date();
  ...
  // --- FORMATO DE REPRESENTACIÓN ---
  {
    ...
    description:
      "Representa la fecha en formato estándar UTC ISO 8601 (ej. YYYY-MM-DDTHH:mm:ss.sssZ).",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
      return parsed.date.toISOString();
  ...
  // --- OTRAS CONVERSIONES LEGÍTIMAS DE ARQUITECTURA ---
  {
    ...
    category: "formatting",
    description:
      "Conserva el texto de fecha y hora eliminando sufijos de zona horaria (Z, UTC, GMT, +02:00, etc.).",
    apply: (input: string) => {
  ...
  },
  {
    ...
    category: "formatting",
    description:
      "Convierte la fecha al horario local de Portugal y elimina el offset de zona horaria para almacenamiento en HOST.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
  ...
  },
  {
    ...
    category: "formatting",
    description:
      "Convierte la fecha al horario local de España y la formatea como YYYY-MM-DD HH:mm:ss.",
    apply: (input: string) => {
      const parsed = parseFlexibleDate(input);
      if (!parsed.date) return input;
