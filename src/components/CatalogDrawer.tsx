...
export const CatalogDrawer: React.FC<CatalogDrawerProps> = ({ isOpen }) => {
  ...
  const formatFunctionCode = (fnStr: string) => {
    ...
    if (minIndent > 0 && minIndent !== Infinity) {
      return lines
        .map((line, i) =>
          i === 0 ? line
          : line.startsWith(" ".repeat(minIndent)) ? line.slice(minIndent)
          : line,
        )
        .join("\n");
    }
  ...
  const categories: { id: string; label: string; icon: React.ReactNode }[] = [
    { id: "all", label: "Todo", icon: <SlidersHorizontal className="w-3 h-3" /> },
    { id: "timezone", label: "Z. Horaria", icon: <Globe className="w-3 h-3" /> },
    { id: "time_presence", label: "Presencia Hora", icon: <Clock className="w-3 h-3" /> },
    { id: "representation", label: "Formatear", icon: <FileCode className="w-3 h-3" /> },
    { id: "formatting", label: "F. Capa", icon: <Server className="w-3 h-3" /> },
  ];
            ...
            {isFilterActive && (
              <div className="mb-2 px-2.5 py-1.5 bg-indigo-950/50 border border-indigo-800/40 rounded-lg flex items-center justify-between text-xs font-semibold text-indigo-300 animate-in fade-in duration-150">
                ...
                <span className="text-[10px] text-slate-400 font-mono font-normal">
                  ...
                  {filteredCatalog.length === 1 ? "conversión" : "conversiones"})
                </span>
              </div>
            )}
              ...
              {filteredCatalog.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between gap-1">
                    <span
                      className="text-[11px] font-bold font-mono text-indigo-300 truncate"
                      title={item.label}
                    >
                      {item.label}
                    </span>
        ...
        >
          ...
          {showCode && (
            <div className="mt-0.5">
              ...
              <pre className="font-mono text-[10.5px] text-emerald-300 bg-slate-900/90 p-2 rounded-lg border border-slate-800/90 whitespace-pre-wrap break-all leading-snug">
